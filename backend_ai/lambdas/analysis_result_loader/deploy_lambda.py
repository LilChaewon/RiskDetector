"""Deploy analysis_result_loader Lambda."""
...
def ensure_function_exists(function_name: str, zip_path: Path, env: dict[str, str], env_vars: dict[str, object]) -> None:
    ...
    role_arn = values.get("LAMBDA_ROLE_ARN", "").strip() or reference["Role"]
    runtime = values.get("LAMBDA_RUNTIME", "").strip() or reference.get("Runtime") or "python3.14"
    timeout = int(values.get("LAMBDA_TIMEOUT", "").strip() or reference.get("Timeout") or 90)
    memory_size = int(values.get("LAMBDA_MEMORY_SIZE", "").strip() or reference.get("MemorySize") or 512)
    ...
    client.create_function(
        ...
        Runtime=runtime,
        Role=role_arn,
        Handler="lambdas.analysis_result_loader.handler.lambda_handler",
...
def deploy() -> None:
    ...
    env_vars = {
        "Variables": {
            ...
            "DB_SSLMODE": values.get("DB_SSLMODE", "disable"),
            "DB_TABLE_NAME": values.get("DB_TABLE_NAME", "analysis_results"),
        }
    }
    ...
    run(
        [
            ...
            REGION,
            "--handler",
            "lambdas.analysis_result_loader.handler.lambda_handler",
            "--environment",
